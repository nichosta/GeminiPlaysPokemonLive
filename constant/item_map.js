...
// This is directly from pokefirered\include\constants\items.h (slightly edited)
const ITEM_NAMES = `
#define ITEM_NONE 0
...
#define ITEM_TM22_SOLAR_BEAM 310
...
/**
 * Parses C preprocessor item definitions into a JavaScript Map.
 * @param {string} definitionString - The string containing the #define statements.
 * @returns {Map<number, string>} - A Map where keys are item IDs (numbers)
 *                                   and values are item names (strings, e.g., "MASTER_BALL").
 */
function parseItemDefinitions(definitionString) {
    const map = new Map();
    const lines = definitionString.split('\n');
    // Regex to capture ABILITY_NAME and ID
    const defineRegex = /^#define\s+ITEM_(\w+)\s+(\d+)/;
    ...
    for (const line of lines) {
        ...
        if (trimmedLine.startsWith('#define')) {
            ...
            if (match) {
                // match[1] is the captured name (e.g., "MASTER_BALL")
                // match[2] is the captured ID string (e.g., "1")
                const itemName = match[1];
                const itemId = parseInt(match[2], 10);

                if (!isNaN(itemId)) {
                    map.set(itemId, itemName);
                } else {
                    console.warn(`Could not parse item ID from line: ${trimmedLine}`);
                }
            }
        }
    }
...
const itemMap = parseItemDefinitions(ITEM_NAMES);

/**
 * @description Gets the item name from the item ID.
 * @param {number} itemId - The item ID.
 * @returns {string | undefined} The item name or undefined if not found.
 */
