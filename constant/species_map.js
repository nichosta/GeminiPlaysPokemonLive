...
// This is directly from pokefirered\include\constants\species.h (slightly edited)
const SPECIES_NAMES = `
...
// Old UNOWN species info went here
...
/**
 * Parses C preprocessor species definitions into a JavaScript Map.
 * @param {string} definitionString - The string containing the #define statements.
 * @returns {Map<number, string>} - A Map where keys are species IDs (numbers)
 *                                   and values are species names (strings, e.g., "BULBASAUR").
 */
function parseSpeciesDefinitions(definitionString) {
    const map = new Map();
    const lines = definitionString.split('\n');
    // Regex to capture SPECIES_NAME and ID
    const defineRegex = /^#define\s+SPECIES_(\w+)\s+(\d+)/;
    ...
    for (const line of lines) {
        ...
        if (trimmedLine.startsWith('#define')) {
            ...
            if (match) {
                // match[1] is the captured name (e.g., "BULBASAUR")
                // match[2] is the captured ID string (e.g., "1")
                const speciesName = match[1];
                const speciesId = parseInt(match[2], 10);

                if (!isNaN(speciesId)) {
                    map.set(speciesId, speciesName);
                } else {
                    console.warn(`Could not parse species ID from line: ${trimmedLine}`);
                }
            }
        }
    }
...
const speciesMap = parseSpeciesDefinitions(SPECIES_NAMES);

/**
 * @description Gets the species name from the species ID.
 * @param {number} speciesId - The species ID.
 * @returns {string | undefined} The species name or undefined if not found.
 */
