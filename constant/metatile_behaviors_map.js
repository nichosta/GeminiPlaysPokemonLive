...
// This is directly from pokefirered\include\constants\metatile_behaviors.h (slightly edited)
const METATILE_BEHAVIORS = `
 ...
 * @returns {Map<number, string>} - A Map where keys are metatile behavior IDs (numbers)
 *                                   and values are metatile behavior names (strings, e.g., "NORMAL").
 */
function parseMetatileBehaviorDefinitions(definitionString) {
    const map = new Map();
    const lines = definitionString.split('\n');
    // Regex to capture MB_NAME and 0xVALUE
    const defineRegex = /^#define\s+MB_(\w+)\s+(0x[0-9A-Fa-f]+)/;
    ...
    for (const line of lines) {
        ...
        if (trimmedLine.startsWith('#define')) {
            ...
            if (match) {
                // match[1] is the captured name (e.g., "NORMAL")
                // match[2] is the captured ID string (e.g., "0x00")
                const behaviorName = match[1];
                const behaviorId = parseInt(match[2], 16); // Parse as hexadecimal

                if (!isNaN(behaviorId)) {
                    map.set(behaviorId, behaviorName);
                } else {
                    console.warn(`Could not parse metatile behavior ID from line: ${trimmedLine}`);
                }
            }
        }
    }
...
const metatileBehaviorMap = parseMetatileBehaviorDefinitions(METATILE_BEHAVIORS);
