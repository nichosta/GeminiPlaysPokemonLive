...
import * as ramState from './ramState.js';
...
async function getGameInfoText() {
    ...
    let partyCount = await ramState.getPartyCount();
    let pokemonInfo = [];
    if (partyCount > 0) {
        for (let i = 0; i < partyCount; i++) {
          let pokemon = await ramState.getPokemonData(i);
          pokemonInfo.push(`
...
async function processLLMResponse(llmResponse) {
    ...
    try {
        if (llmResponse.functionCall == null) {
            ...
            return { text: "No tool call in this response. You may disregard this message if this was intentional."};
        }
        ...
        if (responseFunctionCall && typeof responseFunctionCall === 'object' && responseFunctionCall.name && typeof responseFunctionCall.name === 'string' && responseFunctionCall.args) {
            ...
            console.log("Response is invalid. Outputting object:", responseFunctionCall);
...
console.log(`Starting game loop with model ${CONFIGS.GOOGLE_MODEL_NAME}. Delay: ${CONFIGS.LOOP_DELAY_MS}ms`);
        ...
        try {
            ...
            if (!result) {
                console.error("API Error: No response received from Google AI.");
                 ...
                 console.error(`API Error: Prompt blocked. Reason: ${result.promptFeedback.blockReason}`);
                 console.error("Block Details:", result.promptFeedback.safetyRatings);
            } else {
                // Process the valid response
                let parsedResponse = JSON.parse(result.text);

                console.log(`Total tokens used: ${result.usageMetadata.totalTokenCount}`);

                let responseResult = await processLLMResponse(parsedResponse);

                // Add current user turn and model response to history
                googleHistory.push({ role: "user", parts: currentUserPromptParts });
                googleHistory.push({ role: "model", parts: [{ text: parsedResponse.commentary ?? "No thoughts in the previous turn." }] });
